...
// 4. Spustit Background Sync Service (SKRYTĚ)
console.log('🔄 [4/5] Startuji Cloud Sync...');
const syncPath = path.join(process.cwd(), 'scripts', 'service_sync.js');
if (fs.existsSync(syncPath)) {
    const syncer = fork(syncPath, [], {
        cwd: process.cwd(),
        silent: false // Chceme vidět logy syncu v konzoli
    });
    ...
    console.log('      ✅ Sync service běží');
} else {
    console.log('      ⚠️  Sync script nenalezen');
}

// 5. Spustit Next.js Server (v tomto okně - hlavní proces)
console.log('🧠 [5/5] Startuji Next.js server (port ' + LOCAL_PORT + ')...');
console.log('');
console.log('─────────────────────────────────────────────────────────────');
console.log('');
